...
class Supervisor:
    ...
    def __init__(self, logger: logging.Logger,
        ...
        if controller_bank is None:
            self.logger.warning(f"Не задан банк контроллеров для супервизора {self.name}")
            self.controller_bank = None
        else:
            self.controller_bank = controller_bank

        if estimator_bank is None:
            self.logger.warning(f"Не задан банк эстиматоров для супервизора {self.name}")
    ...
    def compute_estimators(self,
        ...
        if self.estimator_bank is None:
            self.logger.warning(f"Попытка вычислить эстиматоры, но банк эстиматоров не задан для супервизора {self.name}")
            return
    ...
    def compute_controllers(self,
        ...
        if self.controller_bank is None:
            self.logger.warning(f"Попытка вычислить контроллеры, но банк контроллеров не задан для супервизора {self.name}")
            return
